...
```
dependencies {
            implementation 'com.github.shuftipro:Android-NFC-AutoCapture:1.0.4'
}
```
3. Add following in project level bulid.gradle:
```
android {
    defaultConfig {
...
## SDK Version:
Currently our updated SDK version is 1.0.4
...
* ## autoCapture
  ...
  Accepted Values: **true**, **false**  
    ...
    AutoCapture provides automatic capturing of required objects. If **autoCapture** value is set to **TRUE** it would capture the image without the need of clicking capture button. AutoCapture is available for face, document/ document- two(passport, id card, driving license, credit or debit card) and address verification service. The user would also be able to capture manually while AutoCapture is set to true. If **autoCapture** is set to **FALSE** then the user must capture the image by clicking capture button. The document will only be captured if it matches the provided country and document type, otherwise error will be shown.
  <br>
...
* ## allow_online
  ...
  This key specifies if the proof needs to be captured. The 1 value means that the user must capture the proof for verification instead of uploading. This parameter also has priority over allow_offline parameter if both are set to 0.
...
* ## allow_offline
  ...
  This key specifies if the proof needs to be uploaded. The 1 value means that the user must upload the proof for verification instead of capturing.
...
* ## Face
  ...
  * <h3> allow_online</h3>
  ...
  This key specifies if the proof needs to be captured. The 1 value means that the user must capture the proof for verification instead of uploading. This parameter, if included inside any verification, has priority over the parameter inside main object. This parameter also has priority over allow_offline parameter if both are set to 0.
  ...
  * <h3> allow_offline</h3>
  ...
  This key specifies if the proof needs to be uploaded. The 1 value means that the user must upload the proof for verification instead of capturing. This parameter, if included inside any verification, has priority over the parameter inside main object.
...
22 Mar 2022     | Added new features
